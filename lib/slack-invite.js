  ...
  .end(function(err, res){
    ...
    if (!ok) {
      if (providedError === 'missing_scope' && needed === 'admin') {
        ...
        fn(new Error('Você recebeu o convite para o slack. Procure por um email de feedback@slack.com.'));
      } else if (providedError === 'already_in_team') {
        fn(new Error(`Você já Recebeu o convite. Entre em https://${org}.slack.com.`));
      } else {
