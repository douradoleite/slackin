...
export default function splash({ name, org, logo, active, total, channels, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' usuários online agora de ',
          dom('b.total', total), ' registrado.'
        ]
        : [dom('b.total', total), ' usuários registrados até o momento.']
    ),
    dom('form',
      ...
      dom('input.form-item type=email placeholder=seuemail '
        + (!iframe ? 'autofocus' : '')),
      dom('button.loading', 'Quero meu convite')
    ),
    !iframe && dom('p.signin',
      'or ',
      dom(`a href=https://${org}.slack.com target=_top`, 'entrar agora'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
